...
const CartItem = ({ cartItems }) => {
	const { increaseQty, descreaseQty, prdctQunatity, onRemoveCartItem } = useStateContext;
	return (
		<>
			{cartItems?.map((item, key) => (
					<div key={key} className='flex py-8 gap-2 md:gap-4 border-b border-gray-700 mx-3 relative flex-col md:flex-row items-center md:items-start'>
						{/* {cartItems.length >= 1 && */}
							...
							<div className='flex gap-8 my-5 md:my-0 w-full justify-center md:justify-start'>
								...
								<div className='flex gap-2'>
									...
									<div className='quantity-wrapper flex gap-3'>
										...
										<span className='digit'> {prdctQunatity} 0</span>
										<span
											className='px-2 border cursor-pointer rounded'
											onClick={increaseQty}>
											+
